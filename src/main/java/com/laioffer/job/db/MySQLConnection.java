...
public class MySQLConnection {
    ...
    public void setFavoriteItems(String userId, Item item) {
        ...
        // 必须写在前面，因为我们用了foreign key: insert history的时候userId和item_id
        // 都必须存在。所以要先把item存入item table和history table中。
        saveItem(item);
        // 如果user_id和item_id是一样的，直接悄悄地ignore就行
        // ↓这样写以后，一个SQL就可以应用于多个statement
        String sql  = "INSERT IGNORE INTO history (user_id, item_id) VALUES (?, ?)";
        try {
            // ↓ 一般都是先写这句话，IDE标红了以后再surround with try / catch
            PreparedStatement statement = conn.prepareStatement(sql);
            // 替换SQL参数。42行“❓”按照从1开始的index去找对应。↓
