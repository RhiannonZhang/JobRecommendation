...
public class SearchServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 读lat & lon两个参数.把string 转换为double
        ...
        List<Item> items = client.search(lat, lon, null);//用默认的developer
        // 设置响应编码。使客户端浏览器，区分不同种类的数据，并根据不同的MIME调用浏览器内不同的程序嵌入模块来处理相应的数据
        // ↓在getWriter()方法被调用之前调用，那么响应的字符编码将仅从给出的内容类型中设置
